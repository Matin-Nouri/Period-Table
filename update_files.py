...
for i in range(118):
    ...
    if name=='posix':
        path_out = f'./downloads/{data["number"]}_{data["name"]}/'
    else:
        path_out = '.\\downloads\\'
    ...
    url = data['source']
    pdfkit.from_url(url, f'{path_out}{data["name"]}_{data["name"]}_wiki.pdf')

    ul = f'https://images-of-elements.com/{str(data["name"]).lower()}.php'
    pdfkit.from_url(ul, f'{path_out}{data["name"]}_{data["name"]}_element.pdf')
